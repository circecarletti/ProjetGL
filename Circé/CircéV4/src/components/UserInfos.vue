    ...
    <div class="user-info-area">
        ...
        <div class="background-area">
            ...
            <div class="informations">
                ...
                <div class="line" v-if="balance !== 'Inconnue'">
                    <span class="label">solde</span> : {{balance}}
                </div>
                <div class="line" v-if="numberOfChildren >= 0">
                    <span class="label">Nombre d'enfants</span> : {{numberOfChildren}}
                </div>
                <div class="line" v-if="status">
                    <span class="label">Statut</span> : {{status}}
                </div>
            </div>
        </div>
    </div>
 ...
 *          status: status du compte ('ok'/'locked').
 *                  Non spécifié pour un administrateur.
 *      }
 */
export default {
    ...
    computed: {
        ...
        balance() {
            if (this.user && typeof this.user.balance === 'number') {
        ...
        status() {
            if (this.user && typeof this.user.status === 'string') {
                if (this.user.status === 'ok') {
                    return 'ok';
                } else if (this.user.status === 'locked') {
                    return 'vérouillé';
                } else {
                    return 'Inconnu';
                }
