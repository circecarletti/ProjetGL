    ...
    methods: {
        onRemove(borrowedId) {
           ...
           // A FAIRE : appeler le bon service de retrait d'un ouvrage de la liste d'emprunt
            sendPost('https://media-orsay-default-rtdb.firebaseio.com/removeBorrowed.json', removePayload).
                then( response => {
        ...
        onBorrowItem() {
            if (this.isIdValidToBorrow) {
                ...
                sendPost('https://media-orsay-default-rtdb.firebaseio.com/borrowResource.json', borrowPayload).
                    then( response => {
                        console.log(response);
