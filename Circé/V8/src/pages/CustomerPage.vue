...
export default {
    ...
    created() {
        ...
        sendGet(`https://orsaymediatheque.herokuapp.com/api/user/adultmember/${custId}`).
            then( response => {
                const customer = response.docs;
                // console.log("user : ", customer);
                if(response.success){
                    const userRequested = {
                        firstName: customer.member.firstname,
                        lastName: customer.member.name,
                        email: customer.id,
                        age: Number(customer.age),
                        numberOfChildren: customer.childlist.length,
                        balance: Number(customer.member.balance),
                    };
