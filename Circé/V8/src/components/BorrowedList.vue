...
export default {
    ...
    mounted() {
        ...
        sendGet(`https://orsaymediatheque.herokuapp.com/api/user/adultmember/loanInfo/${userId}`).
            then( response => {
                if(response.succes){
                    console.log("liste d'emprunts : ", response.docs.member.loan);
                    const items = response.docs.member.loan.idressource.map( fullItem => {
